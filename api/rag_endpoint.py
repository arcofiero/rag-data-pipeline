...
Request flow:
  ...
  5. Call OpenAI chat completion (gpt-4o) with system prompt + context + query.
...
- Query embedding reuses the same model as the ingestion pipeline
  (text-embedding-3-small) — embedding model must match at query time or
  cosine similarity scores are meaningless.
- Source citations include chunk_id, document_id, source, score so the
  caller can verify provenance and filter by source type.
- max_context_chars caps total context fed to the chat model to stay
  within the gpt-4o context window budget (128k tokens; 20k chars is
  conservative and leaves room for system prompt and answer).
- /health endpoint for load balancer health checks — returns 200 with
  dependency status (Pinecone reachable, OpenAI key present).
- All config from environment — no hardcoded values.
- Clients (OpenAI, Pinecone) initialised once at module level (not per
  request) to reuse HTTP connection pools.
"""
...
def embed_query(query: str) -> List[float]:
    ...
    Must use the same model as the ingestion pipeline — model mismatch
    makes cosine similarity scores meaningless.
    Raises HTTPException 502 on OpenAI failure.
    """
    try:
...
def generate_answer(query: str, context: str) -> str:
    """
    Call OpenAI chat completion with the system prompt, retrieved context,
    and user query. Returns the model's answer string.
    ...
    try:
        ...
        logger.error("OpenAI chat completion failed: {}", exc)
